...
internal actual fun maybeSharedMutex(name: String): PowerSyncMutex {
    return NavigatorLocksMutex(name)
}
...
private class NavigatorLocksMutex(
    ...
    override suspend fun acquire(owner: Any?): HeldMutex = withAbortSignal { signal ->
    ...
    override suspend fun tryAcquire(owner: Any?): HeldMutex? {
        return acquireInternal(ifAvailableLockOptions())
    }
    ...
    private suspend fun acquireInternal(options: JsAny): HeldMutex? {
        ...
        locks.request(name, options) { lock ->
            Promise { resolve, _ ->
                acquiredLock.complete(if (lock == null) {
                    resolve(null)
                    null
                } else {
...
private class PromiseBasedHeldMutex(private val resolve: (JsAny?) -> Unit): HeldMutex {
    override fun close() {
        resolve(null)
    }
}
