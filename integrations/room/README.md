...
2. On your `RoomDatabase.Builder`, call `setDriver()` with a PowerSync-enabled driver: 
    ...
    val driver = BundledSQLiteDriver().also {
        it.addPowerSyncExtension() // Extension method by the PowerSync SDK
    }
...
After these steps, you can open your Room database. Then, you can use the following method to obtain
a `PowerSyncDatabase` instance that is backed by Room:
...
val powersync = PowerSyncDatabase.opened(
    ...
    group = PowerSyncDatabase.databaseGroup(logger, "databaseName"),
    logger = Logger,
)
...
2. Listen for Room changes and invoke a helper method to transfer them to PowerSync:
    ...
    yourRoomDatabase.getCoroutineScope().launch {
        ...
        yourRoomDatabase.invalidationTracker.createFlow("users").collect {
