...
class SqlDelightTest {
    @Test
    fun simpleQueries() = databaseTest { powersync ->
    ...
    @Test
    fun writeCreatesCrudEntry() = databaseTest { powersync ->
        val db = TestDatabase(PowerSyncDriver(powersync, this))
        db.todosQueries.create("my title", "my content")
    ...
    @Test
    fun powerSyncUpdatesSqlDelight() = databaseTest { powersync ->
        val db = TestDatabase(PowerSyncDriver(powersync, this))
        turbineScope {
            val turbine = db.todosQueries.all().asFlow().mapToList(currentCoroutineContext()).testIn(this)
    ...
    @Test
    fun sqlDelightUpdatesPowerSync() = databaseTest { powersync ->
        val db = TestDatabase(PowerSyncDriver(powersync, this))
        turbineScope {
    ...
    @Test
    fun testTransaction() = databaseTest { powersync ->
        val db = TestDatabase(PowerSyncDriver(powersync, this))
        turbineScope {
            val turbine = powersync.watch("SELECT title FROM todos") { it.getString(0)!! }.testIn(this)
            turbine.awaitItem() shouldBe emptyList()
...
private fun databaseTest(body: suspend TestScope.(PowerSyncDatabase) -> Unit) {
    runTest {
        ...
        val db = PowerSyncDatabase(
            ...
            schema = Schema(Table("todos", listOf(
                Column.text("title"),
                Column.text("content"),
            ))),
